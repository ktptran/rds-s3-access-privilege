...
def get_iam_actions(iam, input_data, rds_instances):
    """
    ...
    try:
        for statement in policy_version['PolicyVersion']['Document']['Statement']:
            ...
            # Retrieves all actions that are pertaining to RDS
            action = statement['Action']
            if isinstance(action, list):
                for spec_action in action:
                    if spec_action.startswith('rds:'):
                        action_list.append(spec_action)
            else:
                if action.startswith('rds:') or action == '*':
            ...
            if action_list:
                ...
                if statement["Resource"] == '*':
                    payload['Resource'] = rds_instances
                else:
...
def retrieve_data():
    ...
    environment to see what RDS policies are given to the users
    and groups.

    Returns
    -------
    iam_rds_access: dictionary
        All attached policies within a user's AWS environment
        that gives access to RDS.
    """
    rds = boto3.client('rds')
    rds_instances = []
    for instance in rds.describe_db_instances()['DBInstances']:
        rds_instances.append(instance['DBInstanceArn'])
    ...
    for curr_policy in used_policies:
        ...
        try:
            input_data = get_iam_actions(iam, input_data, rds_instances)
            ...
            if ('Users' in input_data or 'Groups' in input_data) and \
                ...
                iam_rds_access['Policies'].append(input_data)
        except ValueError as err:
            print(f"Error occurred: {err}")
    return iam_rds_access


if __name__ == '__main__':
    policy_data = retrieve_data()
    with open("output/iam.json", "w") as outfile:
